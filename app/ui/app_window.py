...
class AppWindow:
    ...
    def _build_tab_project(self) -> None:
        ...
        # touche Suppr : suppression rapide depuis la liste
        self.files_list.bind("<Delete>", lambda _e: self.delete_selected_pdfs())

        btns = ttk.Frame(frm)
        btns.pack(anchor="w", pady=(6, 0))
        ttk.Button(btns, text="Supprimer PDF sélectionné…", command=self.delete_selected_pdfs).pack(side="left")
...
<<<<<<< HEAD
                            ...
                            k = a.get("kind")
                            if k in ("score_circle", "manual_score"):
                                rad = 0.0
                                try:
                                    rad = float((a.get("style") or {}).get("radius_pt", 9.0) or 9.0)
                                except Exception:
                                    rad = 9.0
                                a["x_pt"] = float(self._corr_margin_x_pt(pi, radius_pt=rad))
                            elif k == "image":
                                rect = a.get("rect")
                                if isinstance(rect, (list, tuple)) and len(rect) == 4:
...
<<<<<<< HEAD
                    ...
                    if sub == "Correction V0" and self._corr_align_margin_enabled():
                        try:
                            rect = ann.get("rect")
                            if isinstance(rect, (list, tuple)) and len(rect) == 4:
                                ann["rect"] = self._align_image_rect_center_to_margin(start_page, rect)
                        except Exception:
                            pass
...
<<<<<<< HEAD
        for info in layout:
            try:
                ...
                pi = 0
            try:
                x_px = float(self._corr_margin_x_pt(pi, radius_pt=9.0)) * zoom + float(info.get("x0", 0.0) or 0.0)
            except Exception:
                continue
            try:
=======
...
>>>>>>> 6f30ad2c70120348bfa3241d6c4cf59172ba1549
    ...
    def delete_selected_pdfs(self) -> None:
        """Supprime un ou plusieurs PDF du projet (avec confirmation).

        - supprime aussi les variantes générées (marge/corrigé) liées au document
        - supprime toutes les marques/annotations associées au document
        """
        if not self._require_project():
            return
        assert self.project is not None

        sel = list(self.files_list.curselection() or [])
        if not sel:
            messagebox.showinfo("Supprimer", "Sélectionne d'abord un PDF dans la liste.")
            return
        ...
        for idx in sel:
            if 0 <= idx < len(self._doc_ids):
                ...
                try:
                    names.append(self.project.get_doc(doc_id).original_name)
                except Exception:
                    names.append(doc_id)
        ...
        msg = (
            "Tu vas supprimer définitivement le(s) PDF suivant(s) du projet :\n\n"
            f"{listing}\n\n"
            "Cela supprimera aussi :\n"
            "- les variantes générées (marge / corrigé)\n"
            "- toutes les marques / annotations associées\n\n"
            "Cette action est irréversible."
        )
        if not messagebox.askyesno("Supprimer PDF", msg, icon="warning"):
            return
        ...
        try:
            ...
            # si save échoue, on tente quand même de rafraîchir l'UI
            pass
        ...
        if self.project.current_doc_id:
            try:
                self._open_doc_in_viewer(self.project.current_doc_id)
            except Exception:
                pass
        else:
            try:
...
<<<<<<< HEAD
    def _page_width_pt(self, page_index: int | None = None) -> float | None:
        ...
        """
        v = getattr(self, 'viewer', None)
        layout = getattr(v, '_layout', None) if v is not None else None
        ...
        # layout peut être une liste indexée, mais on reste robuste via page_index
        try:
            for info in layout:
                if int(info.get('page_index', -1)) == int(page_index):
                    w = float(info.get('w_pt', 0.0) or 0.0)
                    return w if w > 0 else None
        except Exception:
            pass
        return None

    def _clamp_x_pt_to_page(self, page_index: int | None, x_pt: float, *, radius_pt: float = 0.0) -> float:
        """Clamp x_pt pour rester dans la largeur de la feuille.

        On enlève un petit 'safe' (radius + 1pt) pour éviter que le cercle ne dépasse.
        """
        w_pt = self._page_width_pt(page_index)
        if w_pt is None:
            return float(x_pt)
        try:
            safe = max(1.0, float(radius_pt) + 1.0)
        except Exception:
            safe = 1.0
        max_x = max(0.0, float(w_pt) - safe)
        if x_pt > max_x:
            return float(max_x)
        return float(x_pt)

    def _clamp_rect_x_to_page(self, page_index: int | None, x0: float, x1: float) -> tuple[float, float]:
        """Clamp un rectangle horizontalement pour rester dans la largeur de la feuille.

        - On conserve la largeur du rectangle en le décalant si possible.
        - Si le rectangle est plus large que la page, on le borne à [0, w_pt].
        """
        w_pt = self._page_width_pt(page_index)
        if w_pt is None:
        ...
        width = x1f - x0f
        if width <= 0:
            return float(x0f), float(x1f)

        # Trop large : on borne à la page
        if width >= float(w_pt):
            return 0.0, float(w_pt)

        # Décale pour rester dans [0, w_pt]
        if x0f < 0.0:
            x1f -= x0f
            x0f = 0.0
        if x1f > float(w_pt):
            shift = x1f - float(w_pt)
            x0f -= shift
            x1f -= shift
            if x0f < 0.0:
                x0f = 0.0
                x1f = min(float(w_pt), width)

        return float(x0f), float(x1f)

    def _align_image_rect_center_to_margin(self, page_index: int | None, rect: list[float] | tuple[float, float, float, float]) -> list[float]:
        """Aligne une annotation image dans la marge, en considérant le centre de l'image.

        On aligne le centre X du rectangle sur la distance 'Distance marge (cm)' depuis le bord gauche,
        puis on 'clamp' le rectangle pour rester dans la largeur de la feuille.
        """
        if not isinstance(rect, (list, tuple)) or len(rect) != 4:
            return list(rect) if isinstance(rect, (list, tuple)) else [0.0, 0.0, 0.0, 0.0]
        ...
        w = x1f - x0f
        if w == 0.0:
            return [float(x0f), float(y0), float(x1f), float(y1)]

        target_cx = float(self._corr_margin_x_pt(page_index, radius_pt=0.0))
        new_x0 = target_cx - (w / 2.0)
        new_x1 = new_x0 + w

        new_x0, new_x1 = self._clamp_rect_x_to_page(page_index, new_x0, new_x1)
        return [float(new_x0), float(y0), float(new_x1), float(y1)]

    def _corr_margin_x_pt(self, page_index: int | None = None, *, radius_pt: float = 0.0) -> float:
        """X (en points PDF) pour aligner une marque à la distance choisie depuis le bord gauche.

        La valeur est clampée pour rester dans la largeur de la page (évite de sortir de la feuille).
        """
        cm = self._corr_margin_cm()
...
>>>>>>> 6f30ad2c70120348bfa3241d6c4cf59172ba1549
    ...
    def _on_pdf_context_menu(self, page_index: int, x_pt: float, y_pt: float, x_root: int, y_root: int) -> None:
        ...
        # Evite la fuite de ressources Tk (Windows) : si on recrée des menus sans les détruire,
        # on finit par atteindre la limite "No more menus can be allocated".
        # On détruit donc explicitement l'ancien menu (si présent) et on détruit aussi
        # le menu courant dès qu'il est fermé (unposted).
        try:
            old_menu = getattr(self, "_pdf_ctx_menu", None)
        ...
        self._pdf_ctx_menu = menu

        def _destroy_ctx_menu(_evt=None, _menu=menu):
            try:
                if getattr(self, "_pdf_ctx_menu", None) is _menu:
                    self._pdf_ctx_menu = None
            except Exception:
                pass
            try:
                _menu.unpost()
        ...
        try:
            menu.bind("<Unmap>", _destroy_ctx_menu)
        except Exception:
            pass
